...
void ListaAviones::mostrarAvionesDisponibles() {
    if (!avionesDisponibles) {
        ...
        archivoDOT << "digraph AvionesDisponibles {" << std::endl;
        archivoDOT << "rankdir=LR;" << std::endl;
        archivoDOT << "node [shape=plaintext];" << std::endl;

        NodoAvion* actual = avionesDisponibles;
        do {
            archivoDOT << "node" << actual->avion->numero_de_registro << " [label=<" << std::endl;
            archivoDOT << "<table border=\"0\" cellborder=\"1\" cellspacing=\"0\">" << std::endl;
            archivoDOT << "<tr><td align=\"center\"><b>Vuelo: " << actual->avion->vuelo << "</b></td></tr>" << std::endl;
            archivoDOT << "<tr><td>Registro: " << actual->avion->numero_de_registro << "</td></tr>" << std::endl;
            archivoDOT << "<tr><td>Modelo: " << actual->avion->modelo << "</td></tr>" << std::endl;
            archivoDOT << "<tr><td>Fabricante: " << actual->avion->fabricante << "</td></tr>" << std::endl;
            archivoDOT << "<tr><td>Año de fabricación: " << actual->avion->ano_fabricacion << "</td></tr>" << std::endl;
            archivoDOT << "<tr><td>Peso máximo de despegue: " << actual->avion->peso_max_despegue << "</td></tr>" << std::endl;
            archivoDOT << "<tr><td>Aerolínea: " << actual->avion->aerolinea << "</td></tr>" << std::endl;
            archivoDOT << "<tr><td>Estado: " << actual->avion->estado << "</td></tr>" << std::endl;
            archivoDOT << "</table>" << std::endl;
            archivoDOT << ">];" << std::endl;
        ...
        std::string comandoDotToPng = "dot -Tpng AvionesDisponibles.dot -o AvionesDisponibles.png";
        std::cout << "Comando DOT a PNG: " << comandoDotToPng << std::endl; // Debugging

        int resultado = system(comandoDotToPng.c_str());
        if (resultado != 0) {
            std::cerr << "Error al generar el archivo PNG desde DOT para aviones disponibles." << std::endl;
        } else {
            std::cout << "Archivo PNG para aviones disponibles generado exitosamente: AvionesDisponibles.png" << std::endl;
        }
    }
}
...
void ListaAviones::mostrarAvionesMantenimiento() {
    if (!avionesMantenimiento) {
        ...
        archivoDOT << "digraph AvionesMantenimiento {" << std::endl;
        archivoDOT << "rankdir=LR;" << std::endl;
        archivoDOT << "node [shape=plaintext];" << std::endl;
        ...
        // Generar el archivo PNG utilizando Graphviz
        std::string comandoDotToPng = "dot -Tpng AvionesMantenimiento.dot -o AvionesMantenimiento.png";
        std::cout << "Comando DOT a PNG: " << comandoDotToPng << std::endl; // Debugging

        int resultado = system(comandoDotToPng.c_str());
        if (resultado != 0) {
            std::cerr << "Error al generar el archivo PNG desde DOT para aviones en mantenimiento." << std::endl;
        } else {
            std::cout << "Archivo PNG para aviones en mantenimiento generado exitosamente: AvionesMantenimiento.png" << std::endl;
        }
    }
}
...
void ListaAviones::procesarMovimientoAvionPublic(const std::string& archivoMovimientos) {
    procesarMovimientoAvion(archivoMovimientos); // Llama a la implementación privada
}
...
void ListaAviones::procesarMovimientoAvion(const std::string& archivoMovimientos) {
    ...
    while (std::getline(archivo, linea)) {
        // Procesar cada línea del archivo de movimientos
        // Implementa la lógica de procesamiento según sea necesario
        // Por ejemplo, podrías buscar aviones por número de registro
        // y luego moverlos a mantenimiento o disponibilidad según la línea del archivo.
        // Aquí se muestra un ejemplo básico de búsqueda y movimiento:
